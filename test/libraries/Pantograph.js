...
class Pantograph extends Mechanisms{
    ...
    th1 = this.pi/180*angles[0];
    th2 = this.pi/180*angles[1];

    // Forward Kinematics
    let c1 = parseFloat(cos(this.th1));
    let c2 = parseFloat(cos(this.th2));
    let s1 = parseFloat(sin(this.this.th1));
    let s2 = parseFloat(sin(this.th2));
    let xA = l1*c1;
    let yA = l1*s1;
    let xB = d+l2*c2;
    ...
    let h1x = L1*cB * hx/hm; 
    let h1y = L1*cB * hy/hm; 
    let h1h1 = parseFloat(pow(h1x,2)) + parseFloat(pow(h1y,2)); 
    let h1m = parseFloat(sqrt(h1h1)); 
    let sB = parseFloat(sqrt(1-pow(cB,2)));  
    ...
    let phi2 = parseFloat( acos((x_P-this.d-l2*c2)/L2));
